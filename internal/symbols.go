...
func (r *SymbolRepository) watchSymbols(ctx context.Context, updatedCh chan Symbol, deletedCh chan string) {
	...
	for update := range watcher.Updates() {
		...
		case nats.KeyValuePut:
			r.logger.Println("Provider symbol updated:", update.Key())
			symbol := Symbol{}
			err = json.Unmarshal(update.Value(), &symbol)
			if err != nil {
				r.logger.Println("Failed to unmarshal provider symbol:", update.Key(), err)
				continue
			}
			if symbol.ProviderId != update.Key() {
				r.logger.Println("Key mismatch:", update.Key(), symbol.ProviderId)
				continue
			}
			updatedCh <- symbol

		case nats.KeyValueDelete:
			r.logger.Println("Provider symbol deleted:", update.Key())
			deletedCh <- update.Key()

		case nats.KeyValuePurge:
			r.logger.Println("Provider symbol purged:", update.Key())

		default:
			r.logger.Println("Unknown operation on provider symbol:", update.Operation())
		}
	}
}
