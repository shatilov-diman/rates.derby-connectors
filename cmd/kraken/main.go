...
func (p *TickerProcessor) onUpdateSymbol(symbol internal.Symbol) {
	...
	p.symbols[symbol.ProviderId] = symbol
...
func (p *TickerProcessor) onDeleteSymbol(providerSymbolId string) {
	p.symbolsMutex.Lock()
	defer p.symbolsMutex.Unlock()

	_, ok := p.symbols[providerSymbolId]
	if !ok {
		return
	}

	delete(p.symbols, providerSymbolId)

	p.commands <- map[string]any{
		"method": "unsubscribe",
		"params": map[string]any{
			"channel": "book",
			"symbol":  []string{providerSymbolId},
		},
	}
}

func (p *TickerProcessor) getSymbol(providerId string) *internal.Symbol {
	p.symbolsMutex.Lock()
	defer p.symbolsMutex.Unlock()

	symbol, ok := p.symbols[providerId]
	if !ok {
		return nil
	}
...
func (p *TickerProcessor) OnConnect(ctx context.Context) {
	...
	go func() {
		...
		for providerId := range deletedCh {
			p.onDeleteSymbol(providerId)
		}
	}()
}
...
func (p *TickerProcessor) process(_ context.Context, m internal.WSMessage) {
	...
	case "book":
		...
		case "snapshot", "update":
			...
			for _, data := range array {
				...
				symbol := p.getSymbol(dataMap["symbol"].(string))
				if symbol == nil {
					continue
				}
	...
	case "heartbeat":
	default:
		p.logger.Println("Unknown channel:", msg["channel"])
	}
}
